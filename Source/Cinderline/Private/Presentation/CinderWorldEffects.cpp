...
float TerrainZ(const cinder::Simulation& Simulation, float X, float Y, bool bTerrainRelief)
{
    // Canonical geometry alone does not guarantee a bound landscape: a stale bake,
    // absent actor or failed material binding also selects the flat fallback.
    return bTerrainRelief ? CinderLandscapeTerrain::BaselineZ
...
FVector EffectGroundPoint(const cinder::Simulation& Simulation, cinder::Vec2 Point, float Offset,
    bool bTerrainRelief)
{
    return FVector(Point.x, Point.y, TerrainZ(Simulation, Point.x, Point.y, bTerrainRelief) + Offset);
}
    ...
    bool bTerrainRelief)
{
    ...
    float Ground = TerrainZ(Simulation, Point.x, Point.y, bTerrainRelief);
    if (Definition.air)
        Ground = FMath::Max(Ground, CinderLandscapeTerrain::VisualObstacleTopAt(
            Simulation, Point.x, Point.y, bTerrainRelief,
            Definition.radius * 2.0f));
    return FVector(Point.x, Point.y, Ground + Height);
}
...
void UCinderWorldEffects::Update(const cinder::Simulation& Simulation, int32 ViewerTeam, bool bTerrainRelief)
{
    ...
    for (auto It = Simulation.effects().rbegin(); It != Simulation.effects().rend(); ++It)
    {
        ...
        const FVector From = EffectPoint(Simulation, Effect.from, Effect.sourceKind, bTerrainRelief);
        const FVector To = EffectPoint(Simulation, Effect.to, Effect.targetKind, bTerrainRelief);
        ...
        case cinder::EffectType::Impact:
            if (TargetVisible && AreaVisible(Simulation, ViewerTeam, Effect.to, 38.0f))
            {
                ...
                if (bSoftDust) Add(EBatch::Dust,
                    DustTransform(EffectGroundPoint(Simulation, Effect.to, 7.0f, bTerrainRelief), 20.0f + Age * 16.0f));
            }
            break;
        ...
        case cinder::EffectType::Death:
            if (TargetVisible)
            {
                ...
                const float VictimGroundZ = TerrainZ(Simulation, Effect.to.x, Effect.to.y, bTerrainRelief);
    ...
    for (const cinder::Entity& Entity : Simulation.entities())
    {
        ...
        float EntityGroundZ = TerrainZ(Simulation, Entity.pos.x, Entity.pos.y, bTerrainRelief);
        if (Definition.air)
            EntityGroundZ = FMath::Max(EntityGroundZ, CinderLandscapeTerrain::VisualObstacleTopAt(
                Simulation, Entity.pos.x, Entity.pos.y, bTerrainRelief,
                Definition.radius * 2.0f));
        ...
        const float WorkGroundZ = TerrainZ(Simulation, WorkPoint.x, WorkPoint.y, bTerrainRelief);
    ...
    for (int32 Index = Wrecks.Num() - 1; Index >= 0; --Index)
    {
        ...
        Add(EBatch::Scorch, DustTransform(EffectGroundPoint(Simulation, Wreck.Position, 3.0f, bTerrainRelief),
