...
struct FCinderMotionObservation
{
    ...
    /** Display-frame serial: poses still advance between authoritative steps. */
    uint64 Tick = 0;
    /** Fixed simulation tick; the sentinel retains legacy adjacent-frame sampling. */
    uint64 SimulationTick = MAX_uint64;
    /** Online roots interpolate each display frame; local roots change only on simulation steps. */
    bool bInterpolatedPosition = false;
    float Time = 0;
};
...
struct FCinderMotionMemory
{
    ...
    float GaitPhase = 0;
    float WeaponYaw = 0;
    /** Last measured local velocity/work state, retained only between authoritative steps. */
    cinder::Vec2 MovementVelocity{};
