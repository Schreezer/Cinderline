...
constexpr uint32 CliffProfileVersion = 7;

/** Low terrain support beneath each canonical layout's authored rock bodies. */
constexpr float AuthoredCliffFoundationHeight = 40.0f;
/** Conservative world-space top of the full-height authored or fallback rock body. */
constexpr float AuthoredCliffBodyMaxHeight = 340.0f;
...
/** Shared deterministic bake/scenery policy for canonical indexed rectangles at every
 *  match size. Custom mismatches retain legacy bodies; never depends on loaded assets. */
CINDERLINE_API bool UsesAuthoredCliffBody(const cinder::Simulation& Simulation, int32 ObstacleIndex);
/**
 * Conservative obstacle-top envelope for aircraft, in world centimetres, zero far from
 * obstacles. Includes authored bodies and landscape cap dressing, with a smooth approach
 * outside the obstacle. FootprintRadius expands the fully cleared area for the aircraft's
 * hull. Combine with actual GroundHeight using Max; never use this to seat ground units.
 */
CINDERLINE_API float VisualObstacleTopAt(const cinder::Simulation& Simulation, float X, float Y,
    bool bTerrainRelief, float FootprintRadius = 0.0f);
/**
...
/** Map-authored segments for ground paint; never derives paths from hidden entities. */
CINDERLINE_API void GatherPathways(const cinder::Simulation& Simulation,
...
/** Selects a compatible Landscape, or generates matching relief for authored terrain. */
...
private:
    // A CPU-only description also lets headless automation verify the fallback's actual
    // geometry without initializing static-mesh render resources under NullRHI.
    static void BuildFallbackMeshDescription(const cinder::Simulation& Simulation,
        FMeshDescription& OutMesh);
    bool UpdateFallback(const cinder::Simulation& Simulation, UTexture2D* FogMask,
        UTexture2D* TerrainLayers);
    bool BindMaterialTextures(ALandscape* Landscape, UTexture2D* FogMask, UTexture2D* TerrainLayers,
        float WorldSizeInverse, bool bAuthoredMap);
    void HideAll();
    ...
    UPROPERTY(Transient) TObjectPtr<UStaticMeshComponent> FallbackComponent;
    UPROPERTY(Transient) TObjectPtr<UMaterialInstanceDynamic> FallbackMaterial;
    UPROPERTY(Transient) TMap<uint64, TObjectPtr<UStaticMesh>> FallbackMeshes;
    UPROPERTY(Transient) TObjectPtr<UTexture2D> FallbackFogMask;
    UPROPERTY(Transient) TObjectPtr<UTexture2D> FallbackTerrainLayers;
    TSet<uint64> FailedFallbackSignatures;
    float FallbackWorldSize = 0.0f;
    bool bTriedFallbackMaterial = false;
    bool bInitialized = false;
    int32 ActiveMap = INDEX_NONE;

    friend class FCinderAuthoredTerrainFallbackGeometryTest;
    friend class FCinderAuthoredTerrainFallbackLifecycleTest;
};
