...
struct RunResult {
    ...
    std::uint64_t marchWorkerCommands = 0;
...
void keepUnitsMoving(Simulation& simulation, RunResult& result, const TeamState& state, int team,
                     bool includeWorkers, bool includeCombat = true) {
    ...
    for (const auto& candidate : simulation.entities()) {
        if (!candidate.alive() || candidate.team != team || definition(candidate.kind).building ||
            ...
            (candidate.kind == Kind::Worker ? !includeWorkers : !includeCombat)) continue;
    ...
    for (std::size_t group = 0; group < groups.size(); ++group) if (!groups[group].empty()) {
        auto& commandCount = includeCombat ? result.preparationPatrolCommands : result.marchWorkerCommands;
        if (commandCount >= PreparationPatrolCommandLimit) {
            result.valid = false;
            result.errors.push_back(includeCombat ? "preparation patrol command limit exceeded" :
                                                   "march worker holding command limit exceeded");
            return;
        }
        Command move; move.type = CommandType::Move; move.team = team;
        move.units = std::move(groups[group]); move.point = group == 0 ? state.patrolA : state.patrolB;
        if (issue(simulation, result, std::move(move))) ++commandCount;
    }
}
...
std::uint64_t hashRecording(const std::vector<RecordedCommand>& recording) {
    ...
    for (const auto& entry : recording) {
        ...
        add(static_cast<int>(entry.command.queueMode));
        add(static_cast<int>(entry.command.spacing)); add(entry.command.hasArrivalFacing);
        addFloat(entry.command.arrivalFacing);
...
RunResult run(int players, bool profile, bool snapshots, const std::string& diagnosticSavePrefix = {}) {
    ...
    for (int tick = 0; tick < PreparationLimitTicks && simulation.winner() == -1; ++tick) {
        if (tick % CadenceTicks == 0) {
            ...
            bool complete = true;
            for (int team = 0; team < players; ++team)
                complete = complete && teamPrepared(simulation, team);
            if (complete) {
    ...
    if (result.prepared) {
        ...
        {
            // Preparation can finish on the step that funds the last worker's
            // delivery. Keep every worker combat-suppressed during this isolated
            // march without rewriting any measured army destination.
            for (int team = 0; team < players && result.valid; ++team)
                keepUnitsMoving(simulation, result, teams[team], team, true, false);
            if (!result.valid) break;
                    ...
                    if (!definition(entity->kind).air) {
                        ...
                        if (!fresh.points.empty())
                            detail << " fresh_static_route_endpoint=" << fresh.points.back().x << ','
                                   << fresh.points.back().y;
                        detail << " reload_valid=" << (diagnosticSavePrefix.empty() ? "not_checked" :
                                                        (reloadValid ? "true" : "false"));
                        if (reloadValid) {
...
void print(const RunResult& result, const RunResult& repeat) {
    const bool deterministic = result.stateHash == repeat.stateHash &&
                               result.trajectoryHash == repeat.trajectoryHash &&
                               result.recordingHash == repeat.recordingHash &&
                               result.prepared == repeat.prepared && result.winner == repeat.winner;
    const bool success = result.valid && result.prepared && repeat.valid && repeat.prepared && deterministic;
    std::cout << "{\n  \"schema\": \"cinderline.match_baseline.v1\",\n"
              ...
              << "  \"march_worker_holding_commands\": " << result.marchWorkerCommands << ",\n"
    ...
    std::cout << ",\n"
              ...
              << "  \"repeat_trajectory_hash\": \"" << hex(repeat.trajectoryHash) << "\",\n"
              << "  \"recording_hash\": \"" << hex(result.recordingHash) << "\",\n"
              << "  \"repeat_recording_hash\": \"" << hex(repeat.recordingHash) << "\",\n"
              << "  \"recorded_commands\": " << result.recordedCommands << ",\n"
              << "  \"deterministic_repeat\": " << (deterministic ? "true" : "false") << ",\n"
    ...
    if (!deterministic || !repeat.valid || !repeat.prepared) {
        if (!result.errors.empty()) std::cout << ", ";
        std::cout << "\"deterministic repeat diverged or failed validation\"";
    }
    std::cout << "]\n}\n";
}
...
int main(int argc, char** argv) {
    int players = 0; bool profile = false, snapshots = false; std::string diagnosticSavePrefix;
    ...
    RunResult repeat = run(players, false, false);
    print(measured, repeat);
    return measured.valid && measured.prepared && repeat.valid && repeat.prepared && measured.stateHash == repeat.stateHash &&
           measured.trajectoryHash == repeat.trajectoryHash && measured.recordingHash == repeat.recordingHash ? 0 : 1;
}
