...
void Simulation::updateAI() {
    ...
    for (const AISnapshot& worker : own) {
        ...
        const bool rebalance = (tick_ / 40) % 8 == 0 && reassignedMiners < 2 &&
    ...
    if (difficulty == AIDifficulty::Expert && baseThreat == nullptr &&
        ...
        for (const AISighting& site : aiSightings()) {
            if (site.kind != Kind::Headquarters ||
            ...
            const float distance = distanceSquared(home, site.pos);
        ...
        if (raidSite != nullptr) {
            ...
            if (raiders.size() == 3) {
                ...
                sendGroup(CommandType::AttackMove, toSend, raidSite->pos, "raiding a scouted expansion");
