#!/usr/bin/env python3
"""Capture scripted campaign HUD fixtures serially; no physical-input claim."""
import hashlib
import json
import os
...
def protected_hashes():
    campaign_files = list((ROOT / "Saved/Campaign").rglob("*")) if (ROOT / "Saved/Campaign").exists() else []
    return {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
            if p.exists() else None for p in PROTECTED + [p for p in campaign_files if p.is_file()]}
...
selected_cases = set(filter(None, os.environ.get("CINDERLINE_CAPTURE_CASES", "").split(",")))
results = []
try:
    for device, width, height in [("mobile", 1600, 738), ("desktop", 1600, 900)]:
        for mode in (["menu", "worker", "train", "pause", "defeat", "victory", "finale"] if device == "mobile" else ["menu"]):
            name = f"{device}-{mode}"
            if selected_cases and name not in selected_cases:
                continue
            state = mode
            runtime_log = OUTPUT / f"{name}.log"
            source_shot = ROOT / "Saved/CampaignPreview" / f"{state}.png"
            marker = "CINDERLINE_CAMPAIGN_PREVIEW" + f" state={state}"
            preview_command = "cinder.campaignpreview"
            shot_arg = ""
            ...
            command = [str(ROOT / "scripts/unreal.sh"), "play", "-unattended", "-nosound",
                       "-nosplash", "-ForceRes", f"-ExecCmds=t.MaxFPS 30,r.DebugSafeZone.TitleRatio 0.85,{preview_command} {state}{shot_arg}",
                ...
                try:
                    ...
                    while time.time() - started < 120:
                        ...
                        if any(text in log for text in ("Fatal error:", "Assertion failed:",
                                "CINDERLINE_CAMPAIGN_PREVIEW failed=", "CINDERLINE_CAMPAIGN_PREVIEW refused=")):
                        ...
                        if (marker in log
                            ...
                            if stable_polls >= 2:
                                ...
                                markers = [line for line in log.splitlines()
                                           if marker in line]
                                if not markers:
                                    raise RuntimeError("Missing campaign fixture receipt")
            ...
            if asset_hashes() != assets_before:
                raise RuntimeError("Project assets changed during preview capture")
    ...
    (OUTPUT / "preview-verification.json").write_text(json.dumps({
        "schema": "cinderline.campaign_preview.v1", "cases": results,
        "complete": len(results) == 8, "evidence": "scripted Mac rendering; no native gesture or iPhone acceptance",
        "module": str(module.relative_to(ROOT)), "module_sha256": module_sha256,
        "asset_sha256": assets_before, "assets_unchanged": asset_hashes() == assets_before,
