#!/usr/bin/env python3
"""Capture the scripted tactical HUD fixtures serially; no device-input claim."""
import hashlib
import json
import os
...
OUTPUT = ROOT / "artifacts/tactical-orders"
...
try:
    for device, width, height in [("desktop", 1440, 900), ("mobile", 956, 440)]:
        for mode in ["armed", "pending"]:
            name = f"{device}-{mode}"
            state = f"tactical-{mode}"
                        ...
                        if (f"CINDERLINE_MOBILE_HUD_CAPTURE state={state}" in log
                            ...
                            if stable_polls >= 2:
                                ...
                                markers = [line for line in log.splitlines()
                                           if "CINDERLINE_TACTICAL_QUEUE_PREVIEW " in line]
                                if not markers:
    ...
    (OUTPUT / "preview-verification.json").write_text(json.dumps({
        "schema": "cinderline.tactical_preview.v1", "cases": results,
        "complete": len(results) == 4, "evidence": "scripted Mac rendering; no native gesture or iPhone acceptance",
