...
usage() {
  cat <<'USAGE'
Usage: ./scripts/capture-terrain.sh [sector sectorlive map0 map1 map2 fog] [options]
Capture canonical terrain (default: sector, map0, map1, map2, fog at 1440x900).
...
while (( $# )); do
  case "$1" in
    sector|sectorlive|map0|map1|map2|fog)
...
try:
    ...
    if state in ("sector", "sectorlive") and ("CINDERLINE_CANYON_SECTOR_READY buildings=4 mining_workers=3 army=5 ore_nodes=3" not in log or int(fields.get("fog_unknown", 0)) < 1):
