...
{
    // First authored sector: the western expansion of the two-player map zero.
    // Check its rectangle as well as its index so custom snapshot geometry cannot
    // silently lose a heightfield cliff merely because it occupies the same slot.
    if (Simulation.config().map != 0 || Simulation.playerCount() != 2 || ObstacleIndex != 2
        || Simulation.obstacles().size() <= static_cast<size_t>(ObstacleIndex)) return false;
    const float Scale = Simulation.worldSize() / cinder::Simulation::WorldSize;
    const cinder::Obstacle& Obstacle = Simulation.obstacles()[ObstacleIndex];
    return FMath::IsNearlyEqual(Obstacle.center.x, 1320.0f * Scale, 0.01f)
        && FMath::IsNearlyEqual(Obstacle.center.y, 2400.0f * Scale, 0.01f)
        && FMath::IsNearlyEqual(Obstacle.half.x, 380.0f * Scale, 0.01f)
        && FMath::IsNearlyEqual(Obstacle.half.y, 140.0f * Scale, 0.01f);
}
    ...
    bool bTerrainRelief, float FootprintRadius)
{
    ...
    for (int32 Index = 0; Index < static_cast<int32>(Simulation.obstacles().size()); ++Index)
    {
        ...
        const float ObstacleTop = UsesAuthoredCliffBody(Simulation, Index) || !bTerrainRelief
            ? AuthoredCliffBodyMaxHeight : Peak + 120.0f;
        Top = FMath::Max(Top, ObstacleTop * Feather(Distance, Shoulder));
    }
    return Top;
}
...
float HeightAt(const cinder::Simulation& Simulation, float X, float Y)
{
    ...
    for (int32 ObstacleIndex = 0; ObstacleIndex < ObstacleCount; ++ObstacleIndex)
    {
        ...
        if (UsesAuthoredCliffBody(Simulation, ObstacleIndex))
        {
            ...
            // the rectangular tabletop this authored sector is replacing.
