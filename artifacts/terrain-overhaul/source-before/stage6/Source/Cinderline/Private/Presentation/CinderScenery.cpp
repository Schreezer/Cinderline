...
void ForEachTerrainAccent(const cinder::Simulation& Simulation, VisitorType&& Visitor)
{
    ...
    auto VisitGroup = [&](FRandomStream& Random, float X, float Y, bool bShoulder, int32 Group)
    {
    ...
    const std::vector<cinder::Entity>& KnownResources, bool bTerrainRelief)
{
    ...
    for (const cinder::Obstacle& Obstacle : Simulation.obstacles())
    {
        ...
        {
            // One authored formation replaces the landscape wall on the trial obstacle.
            ...
            if (AddBodyPiece(EBatch::CliffMass, 0, 0, LongSize - 8.0f,
                CrossHalf * 2.0f - 8.0f, 65.0f, 0)) ++LastAuthoredCliffObstacles;

            struct FBodyMass
            {
                float Along, Cross, WidthFraction, DepthFraction, Height, Yaw;
            };
            static constexpr FBodyMass Masses[] = {
                {-0.68f, -0.06f, 0.34f, 0.85f, 246.0f, -12.0f},
                {-0.25f,  0.10f, 0.42f, 0.90f, 334.0f,   7.0f},
                { 0.19f, -0.07f, 0.42f, 0.93f, 282.0f,  -9.0f},
                { 0.62f,  0.04f, 0.37f, 0.84f, 224.0f,  14.0f}
            };
            for (int32 Mass = 0; Mass < UE_ARRAY_COUNT(Masses); ++Mass)
            {
                const FBodyMass& Piece = Masses[Mass];
                const float Height = FMath::Clamp(Piece.Height + Random.FRandRange(-5.0f, 5.0f),
                    220.0f, CinderLandscapeTerrain::AuthoredCliffBodyMaxHeight);
                if (!AddBodyPiece(static_cast<EBatch>(Mass), Piece.Along, Piece.Cross,
                    LongSize * Piece.WidthFraction, CrossHalf * 2.0f * Piece.DepthFraction,
                    Height, Piece.Yaw)) continue;
            ...
            // Lower shoulders lean into the main formation and break the straight toe.
            // Their footprints overlap the bodies above; their Z never rises onto a cap.
            if (AddBodyPiece(EBatch::RockE, -0.45f, 0.47f, LongSize * 0.24f,
                CrossHalf * 0.84f, 132.0f, -18.0f)) ++LastAuthoredCliffButtressInstances;
            if (AddBodyPiece(EBatch::RockF, 0.42f, -0.50f, LongSize * 0.25f,
                CrossHalf * 0.86f, 176.0f, 20.0f)) ++LastAuthoredCliffButtressInstances;
        }
