...
bool StartSectorOrders(cinder::Simulation& Sim)
{
    ...
    Patrol.point = {2010, 2370};
    if (!Sim.command(Patrol).accepted) return false;
    UE_LOG(LogCinderCanyonPreview, Display,
        TEXT("CINDERLINE_CANYON_SECTOR_LIVE queued=1 patrol_units=2 destination=(2010,2370) paid_production=1"));
    return true;
}
...
FAutoConsoleCommandWithWorldAndArgs CanyonPreviewCommand(
    ...
    TEXT("DEVELOPMENT: unattended fresh local menu only. Capture map0|map1|map2|fog|sector|sectorlive. sectorlive accepts optional 60..90 frames for fixed-step PNG motion evidence. Uses ordinary vision and simulation updates; never saves."),
    FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
    {
        ...
        const bool bSector = State == TEXT("sector") || bLive;
