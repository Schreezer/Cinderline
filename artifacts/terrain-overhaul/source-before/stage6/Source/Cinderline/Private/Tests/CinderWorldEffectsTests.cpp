...
bool FCinderWorldEffectsFogResetAndStableFrame::RunTest(const FString& Parameters)
{
    ...
    // Canonical coordinates can still render a flat fallback when a Landscape fails
    // binding. Inspect the submitted exhaust transform, not a copy of the height formula.
    Sim.reset({0, 42, false, 1});
    StopStartingWorkers(Sim);
    Fixture.Effects->Reset();
    const Id ModeKite = Sim.debugSpawn(Kind::Kite, 0, Sim.obstacles()[0].center);
    if (!TestTrue(TEXT("Terrain-mode fixture spawns a visible aircraft above a legacy cliff"), ModeKite != 0)) return false;
    TestTrue(TEXT("The effects terrain-mode regression uses canonical simulation geometry"),
    ...
    TestTrue(TEXT("Actual Landscape binding changes exhaust height while canonical geometry stays identical"),
