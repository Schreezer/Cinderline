...
bool FCinderLandscapeHeightGeometryTest::RunTest(const FString& Parameters)
{
    ...
    auto ValidateHeightfield = [this](const cinder::Simulation& Simulation, const FString& Label)
    {
        ...
        TestEqual(*FString::Printf(TEXT("%s opts in only the authored western expansion"), *Label),
            AuthoredBodies, Simulation.config().map == 0 && Simulation.playerCount() == 2 ? 1 : 0);
        ...
        // Heightfield cliffs retain their tall silhouette contract. The authored sector
        // instead requires a low, continuous support with an exact guard: its full-height
        // rock coverage and containment are checked by SceneryObstacleBounds. Keeping a
        // tall mesa here would stack two cliff bodies and restore the rectangular cake.
        for (int32 ObstacleIndex = 0; ObstacleIndex < ObstacleCount; ++ObstacleIndex)
        {
            const cinder::Obstacle& Obstacle = Simulation.obstacles()[ObstacleIndex];
            const bool bAuthored = UsesAuthoredCliffBody(Simulation, ObstacleIndex);
            if (!bAuthored)
                TestTrue(*FString::Printf(TEXT("%s heightfield obstacle %d retains its tall silhouette (%.0f cm)"),
                    *Label, ObstacleIndex, ObstaclePeak[ObstacleIndex]),
                    ObstaclePeak[ObstacleIndex] >= CliffPeakFloor * 0.88f);
            else
            {
                TestEqual(TEXT("Authored rock support reaches its low core and never retains a tall mesa"),
                    ObstaclePeak[ObstacleIndex], AuthoredCliffFoundationHeight);
            ...
            {
                const float ExpectedTop = bAuthored || !bRenderedRelief ? AuthoredCliffBodyMaxHeight
                    : FMath::Clamp(FMath::Min(Obstacle.half.x, Obstacle.half.y) * 2.0f,
                        CliffPeakFloor, MaxRelief) + 120.0f;
                constexpr float HullRadius = 52.0f;
                TestTrue(TEXT("Aircraft clear mesh bodies or landscape dressing in either rendered terrain mode"),
                    VisualObstacleTopAt(Simulation, Obstacle.center.x, Obstacle.center.y,
                        bRenderedRelief, HullRadius) >= ExpectedTop);
                TestTrue(TEXT("Aircraft already clear the cliff before their hull crosses its boundary"),
                    VisualObstacleTopAt(Simulation, Obstacle.center.x - Obstacle.half.x - HullRadius,
                        Obstacle.center.y, bRenderedRelief, HullRadius) >= ExpectedTop - 0.001f);
                if (bAuthored)
                {
                    const float EdgeX = Obstacle.center.x - Obstacle.half.x;
                    const float Shoulder = 520.0f * Simulation.worldSize() / cinder::Simulation::WorldSize;
                    float PreviousTop = ExpectedTop;
                    for (int32 Step = 0; Step <= 32; ++Step)
                    {
                        const float Outside = HullRadius + Shoulder * Step / 32.0f;
                        const float Top = VisualObstacleTopAt(Simulation, EdgeX - Outside,
                            Obstacle.center.y, bRenderedRelief, HullRadius);
                        TestTrue(TEXT("Aircraft descend smoothly away from the authored cliff instead of popping at its edge"),
                            Top <= PreviousTop + 0.001f && Top >= 0.0f);
                        PreviousTop = Top;
                    }
                    TestTrue(TEXT("The aircraft obstacle envelope ends beyond its broad approach shoulder"),
                        PreviousTop < 0.001f);
                }
            }
        }
    ...
    for (int32 Map = 0; Map < MapCount; ++Map)
    {
        ...
        if (Map == 0)
        {
            cinder::net::Snapshot ChangedSector = cinder::net::snapshotFor(Simulation, 0);
            ChangedSector.obstacles[2].half.x += 1.0f;
            cinder::Simulation CustomSector;
            TestTrue(TEXT("A custom western obstacle remains a valid simulation layout"),
                CustomSector.applySnapshot(ChangedSector, &ApplyError));
            TestFalse(TEXT("An altered western obstacle cannot silently receive the authored low support"),
                UsesAuthoredCliffBody(CustomSector, 2));
            TestTrue(TEXT("An altered western obstacle retains the ordinary tall heightfield fallback"),
                HeightAt(CustomSector, ChangedSector.obstacles[2].center.x,
                    ChangedSector.obstacles[2].center.y) >= CliffPeakFloor * 0.88f);
        }
