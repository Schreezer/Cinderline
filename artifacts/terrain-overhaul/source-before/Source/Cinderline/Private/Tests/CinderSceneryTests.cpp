...
bool FCinderSceneryObstacleBoundsTest::RunTest(const FString& Parameters)
{
    ...
    for (MatchLength Length : {MatchLength::Short, MatchLength::Standard, MatchLength::Long})
    {
        ...
        TestEqual(TEXT("Relief uses the continuous landscape cliff instead of a duplicate skirt"),
            Relief.CliffMassInstances, 0);
        ...
        TestEqual(TEXT("Relief dressing retains the twelve fixed batches"), Relief.BatchCount, 12);
