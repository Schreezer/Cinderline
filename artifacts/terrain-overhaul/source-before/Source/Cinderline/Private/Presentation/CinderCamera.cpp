...
ACinderCamera::ACinderCamera()
{
    ...
    Camera->PostProcessSettings.BloomIntensity = 0.52f;
    ...
    // Stylized grade. Saturation and contrast separate small silhouettes from the
    // ground, and the split tone keeps shaded faces blue against a warm key so
    // shape reads by colour as well as by value.
    //
    // The previous values were roughly a third of these and sat below the
    // perceptual threshold at phone size — a 6% contrast lift is invisible on a
    // 40-pixel unit viewed at arm's length. The split tone is the load-bearing
    // part: cool shadows against warm highlights give every curved hull a hue
    // gradient, which survives MetalFX and FXAA far better than a value gradient.
    Camera->PostProcessSettings.bOverride_ColorSaturation = true;
    Camera->PostProcessSettings.ColorSaturation = FVector4(1.24f, 1.24f, 1.24f, 1.0f);
    Camera->PostProcessSettings.bOverride_ColorContrast = true;
    Camera->PostProcessSettings.ColorContrast = FVector4(1.18f, 1.18f, 1.18f, 1.0f);
    ...
    Camera->PostProcessSettings.ColorGainShadows = FVector4(0.78f, 0.90f, 1.22f, 1.0f);
    Camera->PostProcessSettings.bOverride_ColorGainHighlights = true;
    Camera->PostProcessSettings.ColorGainHighlights = FVector4(1.10f, 1.02f, 0.90f, 1.0f);
    ...
    Camera->PostProcessSettings.VignetteIntensity = 0.34f;
...
float ACinderCamera::MaximumDistance(const FGroundFootprint& Footprint) const
{
    ...
    return static_cast<float>(FMath::Max(1.0, FMath::Min(static_cast<double>(ACinderCamera::FarthestDistance), Available / FMath::Max(Span.X, Span.Y))));
}
...
void ACinderCamera::UpdateTargetBounds(const FGroundFootprint& Footprint)
{
    ...
    const float MinDistance = FMath::Min(ACinderCamera::ClosestDistance, MaxDistance);
