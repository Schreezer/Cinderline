...
os_log_t latencySignpostLog() {
    static os_log_t log = os_log_create("com.cinderline.simulation", OS_LOG_CATEGORY_POINTS_OF_INTEREST);
    return log;
}
#endif
...
struct RunResult {
    ...
    bool prepared = false;
    bool valid = true;
    bool diagnosticInterruptedPrefix = false;
    bool earlyEnd = false;
    int winner = -1;
    std::uint64_t preparationTicks = 0;
    std::uint64_t diagnosticStopAfterTicks = 0;
...
void stepStage(Simulation& simulation, RunResult& result, StageMetrics& stage,
...
#endif
    const auto visibilityBefore = simulation.navigationVisibilityStats();
    ...
    if (updateWallMs >= 20.0 || simulation.tick() == 6416 || simulation.tick() == 8827) {
        const auto visibilityAfter = simulation.navigationVisibilityStats();
        ...
        std::cerr << std::fixed << std::setprecision(3)
                  ...
                  << " terrain_hits=" << visibilityAfter.hits - visibilityBefore.hits
                  << " terrain_misses=" << visibilityAfter.misses - visibilityBefore.misses
                  << " terrain_saturation_misses=" << visibilityAfter.saturationMisses - visibilityBefore.saturationMisses
                  << " terrain_resets=" << visibilityAfter.resets - visibilityBefore.resets
                  << " terrain_pages=" << visibilityAfter.pages
                  << " terrain_targets=" << visibilityAfter.targets
                  << " terrain_payload_bytes=" << visibilityAfter.payloadBytes
...
RunResult run(int players, bool profile, bool snapshots, const std::string& diagnosticSavePrefix = {},
    ...
    result.diagnosticStopAfterTicks = diagnosticStopAfterTicks;
    ...
    for (int tick = 0; tick < PreparationLimitTicks && simulation.winner() == -1; ++tick) {
        ...
        if (result.valid && diagnosticStopAfterTicks && simulation.tick() >= diagnosticStopAfterTicks) {
            result.diagnosticInterruptedPrefix = true;
            break;
        }
        if (!result.valid) break;
    }
    ...
    sampleProgress(simulation, result);
    if (!result.prepared && !result.diagnosticInterruptedPrefix) {
        if (result.valid) {
            result.valid = false;
...
void print(const RunResult& result) {
    const bool measuredValid = result.valid && result.prepared && !result.diagnosticInterruptedPrefix;
    std::cout << "{\n  \"schema\": \"cinderline.match_latency_probe.v1\",\n"
              << "  \"success\": false,\n"
              << "  \"diagnostic_only\": true,\n"
              << "  \"full_acceptance\": false,\n"
              << "  \"repeat_run_performed\": false,\n"
              << "  \"completed\": " << (result.diagnosticInterruptedPrefix ? "false" : "true") << ",\n"
              << "  \"diagnostic_interrupted_prefix\": " << (result.diagnosticInterruptedPrefix ? "true" : "false") << ",\n"
              << "  \"diagnostic_stop_after_ticks\": " << result.diagnosticStopAfterTicks << ",\n"
    ...
    std::cout << (result.diagnosticInterruptedPrefix
        ? "\"diagnostic interrupted preparation prefix; full workload criteria and repeat not performed\""
        : "\"diagnostic single measured run; deterministic repeat and full acceptance not performed\"");
    std::cout << "]\n}\n";
}
...
int main(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        else if (argument == "--diagnostic-stop-after-ticks" && index + 1 < argc) {
            const std::string value = argv[++index];
            std::size_t consumed = 0;
            try {
                diagnosticStopAfterTicks = std::stoull(value, &consumed);
            } catch (...) {
                std::cerr << "Invalid diagnostic tick limit: " << value << '\n'; return 2;
            }
            if (!diagnosticStopAfterTicks || consumed != value.size() ||
                diagnosticStopAfterTicks > static_cast<std::uint64_t>(PreparationLimitTicks)) {
                std::cerr << "Diagnostic tick limit must be within 1.." << PreparationLimitTicks << ".\n";
                return 2;
            }
        }
    ...
    RunResult measured = run(players, profile, snapshots, diagnosticSavePrefix, diagnosticStopAfterTicks);
    print(measured);
    return measured.valid && (measured.prepared || measured.diagnosticInterruptedPrefix) ? 0 : 1;
}
