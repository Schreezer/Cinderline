            ...
            for (const auto& target : assigned[team]) {
                ...
                if (distanceSquared(entity->pos, target.goal) > 35.0f * 35.0f && laggardDetails++ < 8) {
                    ...
                    detail << "team " << team << " march laggard id=" << entity->id
                           ...
                           << " path_index=" << entity->pathIndex
                           << " path_size=" << entity->path.size()
                           ...
                           << " navigation_anchor=" << entity->navigationAnchor.x << ',' << entity->navigationAnchor.y
                           << " stalled_for=" << entity->stalledFor
                           << " navigation_best_distance=" << entity->navigationBestDistance
                           << " avoidance_side=" << entity->avoidanceSide
                    ...
                    detail << " path_points=";
                    for (std::size_t point = 0; point < entity->path.size(); ++point) {
                        if (point) detail << ';';
                        detail << entity->path[point].x << ',' << entity->path[point].y;
                    }
                    struct Neighbor { float gap = 0; const Entity* entity = nullptr; };
                    std::vector<Neighbor> neighbors;
                    for (const auto& other : simulation.entities()) {
                        if (!other.alive() || other.id == entity->id || other.team != entity->team ||
                            definition(other.kind).building || other.kind == Kind::Resource || definition(other.kind).air) continue;
                        const float gap = std::sqrt(distanceSquared(entity->pos, other.pos)) -
                            definition(entity->kind).radius - definition(other.kind).radius;
                        if (gap <= 160) neighbors.push_back({gap, &other});
                    }
                    std::sort(neighbors.begin(), neighbors.end(), [](const Neighbor& left, const Neighbor& right) {
                        return left.gap < right.gap || (left.gap == right.gap && left.entity->id < right.entity->id);
                    });
                    detail << " nearby_ground=";
                    for (std::size_t index = 0; index < std::min<std::size_t>(neighbors.size(), 12); ++index) {
                        if (index) detail << ';';
                        const auto& other = *neighbors[index].entity;
                        detail << other.id << ',' << kindName(other.kind) << ',' << neighbors[index].gap << ','
                               << other.pos.x << ',' << other.pos.y << ',' << static_cast<int>(other.order) << ','
                               << other.goal.x << ',' << other.goal.y << ',' << other.yieldFor << ','
                               << other.stalledFor << ',' << other.navigationExhausted;
                    }
...
void print(const RunResult& result) {
    const bool success = result.valid && result.prepared;
    std::cout << "{\n  \"schema\": \"cinderline.crowd_stall_diagnostic.v1\",\n"
