#!/usr/bin/env python3
"""Capture scripted tutorial HUD fixtures serially; no physical-input claim."""
import hashlib
import json
import os
...
OUTPUT = ROOT / "artifacts/android-hud-touch"
...
try:
    for device, width, height in [("mobile", 1600, 738)]:
        for mode in ["train-open", "train-portrait"]:
            name = f"{device}-{mode}"
            state = mode
            runtime_log = OUTPUT / f"{name}.log"
            source_shot = ROOT / "Saved/TutorialMicrosteps" / f"{state}.png"
            ...
            command = [str(ROOT / "scripts/unreal.sh"), "play", "-unattended", "-nosound",
                       "-nosplash", "-ForceRes", f"-ExecCmds=t.MaxFPS 30,r.DebugSafeZone.TitleRatio 0.85,cinder.TutorialCoachPreview {state}",
                ...
                try:
                    ...
                    while time.time() - started < 120:
                        ...
                        if any(text in log for text in ("Fatal error:", "Assertion failed:",
                                "CINDERLINE_TUTORIAL_COACH_PREVIEW failed=", "CINDERLINE_TUTORIAL_COACH_PREVIEW refused=")):
                            raise RuntimeError(f"Preview failed; inspect {runtime_log}")
                        if (f"CINDERLINE_TUTORIAL_COACH_PREVIEW state={state}" in log
                            ...
                            if stable_polls >= 2:
                                ...
                                markers = [line for line in log.splitlines()
                                           if "CINDERLINE_TUTORIAL_COACH_PREVIEW state=" in line]
                                if not markers:
    ...
    (OUTPUT / "preview-verification.json").write_text(json.dumps({
        "schema": "cinderline.android_hud_touch_preview.v1", "cases": results,
