#!/usr/bin/env python3
"""Capture scripted Patrol/Escort HUD fixtures serially; no physical-input claim."""
import hashlib
import json
import os
...
OUTPUT = ROOT / "artifacts/patrol-escort"
...
try:
    for device, width, height in [("desktop", 1440, 900), ("mobile", 956, 440)]:
        for mode in ["patrol", "patrol-pending", "escort", "escort-pending"]:
            name = f"{device}-{mode}"
            state = mode
                        ...
                        if (f"CINDERLINE_MOBILE_HUD_CAPTURE state={state}" in log
                            ...
                            if stable_polls >= 2:
                                ...
                                markers = [line for line in log.splitlines()
                                           if "CINDERLINE_SUSTAINED_ORDER_PREVIEW " in line]
                                if not markers:
    ...
    (OUTPUT / "preview-verification.json").write_text(json.dumps({
        "schema": "cinderline.sustained_preview.v1", "cases": results,
