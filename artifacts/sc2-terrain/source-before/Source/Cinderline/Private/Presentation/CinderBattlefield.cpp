...
void ACinderBattlefield::BeginPlay()
{
    ...
    // Key-to-fill was 2.9:1, which is why nothing in frame had punch: every
    // shaded face was nearly as bright as every lit one, so form disappeared and
    // only silhouette survived. 8.1:1 restores modelling on curved hulls.
    //
    // The azimuth stays 90 degrees off the camera boom's -45 yaw so the key rakes
    // across units rather than flattening them head-on. The pitch is raised from
    // -38 to -46 so shadows shorten by roughly a quarter and sit under a unit
    // instead of streaking away from it, which at this miniature scale is the
    // difference between a grounded model and a decal.
    ...
    SunComponent->SetLightColor(FLinearColor(1.0f, 0.855f, 0.66f));
    ...
    // The fill has one more job than tinting shade: it is the only thing
    // separating an unlit cliff face from the black of unexplored ground. Cut to
    // 1.05 the rock masses went to the same value as the fog and the skyline read
    // as holes, so this sits at 1.30 — still a 6.5:1 key ratio against the old
    // 2.9:1, but with a floor under every shadowed surface.
    SkyComponent->SetIntensity(1.30f);
    SkyComponent->SetLightColor(FLinearColor(0.40f, 0.57f, 1.0f));
...
void ACinderBattlefield::AddEntity(const cinder::Entity& AuthoritativeEntity)
{
    ...
    //
    // Air units keep their 125 cm of clearance ABOVE THE GROUND, not above Z 0. Measured from
    // zero, a gunship would fly straight into the side of a 480 cm obstacle mesa; measured
    // from the ground it climbs the relief the way an aircraft on a fixed altimeter would.
    ...
    float GroundZ = GroundHeight(RenderPoint);
...
void ACinderBattlefield::RenderPoseState()
{
    ...
    for (const FCinderDyingEntity& Wreck : EntityMotion.DyingEntities())
    {
        ...
        const float WreckZ = GroundHeight(Wreck.Position)
            + (cinder::definition(Wreck.Kind).air ? 125.0f : bWreckHover ? 8.0f : 0.0f)
            + DeathPose.BodyZ;
