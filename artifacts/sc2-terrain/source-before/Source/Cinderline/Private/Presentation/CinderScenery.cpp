...
// Ground props sink this far into the surface they stand on. The heightfield the
// landscape lane builds is a smoothed plateau, so a mesa parked exactly on the sampled
// height shows a hairline of ground plane under its skirt wherever the interpolation
// dips between samples. Twenty centimetres is below the silhouette budget of even the
// smallest 240 cm mesa and hides that seam at every camera angle.
    ...
    for (const cinder::Obstacle& Obstacle : Simulation.obstacles())
    {
        ...
        for (int32 Segment = 0; Segment < Segments; ++Segment)
        {
            ...
            for (int32 Row = 0; Row < Rows; ++Row)
            {
                ...
                // The landscape lane raises obstacle plateaus to a 480 cm cap. A 180-240 cm
                // mesa parked at absolute Z would be swallowed whole by its own plateau, so
                // the range moves with it and every instance now stands on the heightfield.
