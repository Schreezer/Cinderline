...
FVector EffectPoint(const cinder::Simulation& Simulation, cinder::Vec2 Point, cinder::Kind Kind)
{
    ...
    float Ground = TerrainZ(Simulation, Point.x, Point.y);
    if (Definition.air)
        Ground = FMath::Max(Ground, CinderLandscapeTerrain::VisualObstacleTopAt(
            Simulation, Point.x, Point.y, CinderLandscapeTerrain::IsCanonicalGeometry(Simulation),
...
void UCinderWorldEffects::Update(const cinder::Simulation& Simulation, int32 ViewerTeam)
{
    ...
    for (const cinder::Entity& Entity : Simulation.entities())
    {
        ...
        float EntityGroundZ = TerrainZ(Simulation, Entity.pos.x, Entity.pos.y);
        if (Definition.air)
            EntityGroundZ = FMath::Max(EntityGroundZ, CinderLandscapeTerrain::VisualObstacleTopAt(
                Simulation, Entity.pos.x, Entity.pos.y, CinderLandscapeTerrain::IsCanonicalGeometry(Simulation),
                Definition.radius * 2.0f));
