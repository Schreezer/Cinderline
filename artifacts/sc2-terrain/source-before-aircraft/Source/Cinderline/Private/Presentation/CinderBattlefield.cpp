...
void ACinderBattlefield::AddEntity(const cinder::Entity& AuthoritativeEntity)
{
    ...
    if (Def.air)
        GroundZ = FMath::Max(GroundZ, CinderLandscapeTerrain::VisualObstacleTopAt(
            *Simulation, RenderPoint.x, RenderPoint.y, bTerrainRelief, Def.radius * 2.0f));
...
void ACinderBattlefield::RenderPoseState()
{
    ...
    for (const FCinderDyingEntity& Wreck : EntityMotion.DyingEntities())
    {
        ...
        float WreckGroundZ = GroundHeight(Wreck.Position);
        if (WreckDef.air)
            WreckGroundZ = FMath::Max(WreckGroundZ, CinderLandscapeTerrain::VisualObstacleTopAt(
                *Simulation, Wreck.Position.x, Wreck.Position.y, bTerrainRelief, WreckDef.radius * 2.0f));
