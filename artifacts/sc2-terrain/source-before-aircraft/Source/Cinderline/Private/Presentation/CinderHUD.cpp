...
void ACinderHUD::RefreshMinimapTerrain(ACinderBattlefield* Battle)
{
    ...
    if (MinimapTerrain && MinimapTerrainMap == Map
...
void ACinderHUD::Button(const FString& Text, const FString& Action, int Arg, float X, float Y, float W, bool Active)
{
    ...
    Label(Text, X + Space2*S, Y + (bBuild ? 7 : 14)*S,
        Lift(Primary ? Ink : Active ? Accent : White, Press * 0.30f), FontScale * Fit);
...
void ACinderHUD::ActionButton(const FString& Text, const FString& Action, int Arg, float X, float Y, float W, bool Active)
{
    ...
    const bool Warning=Action.StartsWith(TEXT("cancel"));
    const FLinearColor Accent=Warning ? Amber : Mint;
    // Drawn rect only: Hit, and therefore FButton::Bounds, is the untouched
    // 44-unit target whatever the animation does to the visible face.
    const float Press=PressPulse(Action,Arg,0);
    const float Pop=Press*1.5f*S;
    const FLinearColor Rim=Active ? Accent.CopyWithNewOpacity(0.55f)
        : FLinearColor(0.21f,0.31f,0.36f,Hover ? 0.5f : 0.24f);
    Surface(X+2*S+Pop,Y+2*S+Pop,W-4*S-2*Pop,H-4*S-2*Pop,
        Lift(Rim,0.10f+Press*0.34f),
        FLinearColor(Rim.R*0.45f,Rim.G*0.45f,Rim.B*0.55f,
            FMath::Min(1.0f,Rim.A+Press*0.20f)),8*S);
    const FLinearColor FaceTop=Active ? FLinearColor(0.024f,0.106f,0.122f,0.96f)
        : FLinearColor(0.019f,0.033f,0.052f,0.91f);
    const FLinearColor FaceBottom=Active ? FLinearColor(0.008f,0.044f,0.052f,0.96f)
        : FLinearColor(0.005f,0.011f,0.020f,0.91f);
    Surface(X+3*S+Pop,Y+3*S+Pop,W-6*S-2*Pop,H-6*S-2*Pop,
        Lift(FaceTop,Press*0.30f),Lift(FaceBottom,Press*0.22f),7*S);
    const FLinearColor Color=Lift(Active ? Accent : White,Press*0.30f);
    const bool Glyph=ActionGlyph(Action,Arg,X+(W-19*S)*0.5f,Y+6*S+Pop,19*S,Color);
    const float FontScale=TypeCaption;
    const float TW=MeasureLabel(Text,FontScale).X;
    const float Fit=TW>0 ? FMath::Min(1.0f,(W-6*S)/TW) : 1;
    Label(Text,X+(W-TW*Fit)*0.5f,Y+(Glyph ? 28 : 17)*S+Pop,Color,FontScale*Fit);
...
void ACinderHUD::DrawWorldIndicators(ACinderPlayerController* PC, ACinderBattlefield* Battle)
{
    ...
    for (const auto& E : Sim.entities())
    {
        ...
        if (Selected || E.hp < D.hp || E.progress < 1)
        {
            ...
            if (PC->ProjectWorldLocationToScreen(Ground(RenderPoint, Z), P))
            {
    ...
    if (!Battle->HasWorldEffects())
    {
        ...
        for (const auto& FX : Sim.effects())
        {
            ...
            const FVector From(FX.from.x, FX.from.y,
                Battle->GroundHeight(FX.from) + EffectHeight(FX.sourceKind));
            const FVector To(FX.to.x, FX.to.y,
                Battle->GroundHeight(FX.to) + EffectHeight(FX.targetKind));
            ...
            else if (FX.type == cinder::EffectType::Death && TargetVisible)
            {
                ...
                const FVector Center(FX.to.x, FX.to.y, Battle->GroundHeight(FX.to)
...
void ACinderHUD::DrawProductionQueue(const cinder::Entity* Producer, float X, float Y, float W, int Columns)
{
    ...
    SingleLineLabel(FString::Printf(TEXT("QUEUE %d / %d"), Count, cinder::Simulation::MaxQueue), X + 5 * S, Y + 3 * S, W - 219 * S, Amber, TypeBody);
#if PLATFORM_IOS || PLATFORM_ANDROID
    ...
    SingleLineLabel(Range, X + 5 * S, Y + 25 * S, W - 219 * S, Muted, TypeCaption);
...
void ACinderHUD::DrawCompactMatch(ACinderPlayerController* PC, ACinderBattlefield* Battle)
{
    ...
    if (PC->IsBuildMode() || PC->IsProductionRallyMode())
    {
        if (PC->IsManualBuildMode() && PC->HasSingleSelectedWorker())
            ActionButton(PC->IsQueueNextArmed() ? TEXT("Queue on") : TEXT("Queue"),
                TEXT("queuenext"), 0, L.Right - 196 * S, DockY, 96 * S, PC->IsQueueNextArmed());
    ...
    {
        const float W=49*S, Gap=Space1*S, X=L.Commands.Min.X;
        if (bCombatSelection)
        {
        ...
        else if(bBuilding)
        {
            ...
            {
                ActionButton(TEXT("Rally"),TEXT("productionrally"),0,X+W+Gap,DockY,W);
                Buttons.Last().EntityId=First->id;
            }
            else ActionButton(TEXT("Focus"),TEXT("focus"),0,X+W+Gap,DockY,W);
            if(bProductionBuilding && !bSite)
            {
    ...
    else if (!PC->IsBuildMode() && (PC->bBuildMenu || CompactSheet != 0))
    {
        ...
        for (int I = 0; I < Options.Num(); ++I)
        {
            ...
            if (Options[I].Action == TEXT("build"))
            {
                ...
                const TCHAR* Availability = bPlanning ? TEXT("PLAN") : Status.accepted ? TEXT("READY") : TEXT("LOCKED");
