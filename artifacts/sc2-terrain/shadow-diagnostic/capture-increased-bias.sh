...
project_root=/Users/chirag13/Documents/ChatGPT/starCraft
...
usage() {
  cat <<'USAGE'
Usage: ./scripts/capture-terrain.sh [map0 map1 map2 fog] [options]
Capture canonical terrain at 1440x900 (default: all four states).
...
(( ${#states[@]} )) || states=(map0 map1 map2 fog)
...
try:
    ...
    expected_map = state[-1] if state != "fog" else "0"
...
for state in "${states[@]}"; do
  ...
  "$command_editor" "$project_root/Cinderline.uproject" -game -unattended \
    ...
    "-ExecCmds=t.MaxFPS 30,cinder.quality,r.Shadow.CSMDepthBias 18,r.Shadow.CSMSlopeScaleDepthBias 6.85,r.Shadow.CSMDepthBias,r.Shadow.CSMSlopeScaleDepthBias,cinder.canyonpreview $state" \
