#!/usr/bin/env python3
"""Capture scripted resource rally HUD fixtures serially; no physical-input claim."""
import hashlib
import json
import os
...
OUTPUT = ROOT / "artifacts/resource-rally"
...
results = []
try:
    for device, width, height in [("mobile", 1600, 738)]:
        for mode in ["producer", "worker-rally"]:
            name = f"{device}-{mode}"
            state = mode
            runtime_log = OUTPUT / f"{name}.log"
            source_shot = ROOT / "Saved" / ("MobileHUD" if mode == "producer" else "ArmyCommand") / f"{state}.png"
            marker = ("CINDERLINE_MOBILE_HUD_CAPTURE" if mode == "producer" else "CINDERLINE_COMMAND_PREVIEW") + f" state={state}"
            preview_command = "cinder.mobilepreview" if mode == "producer" else "cinder.commandpreview"
            shot_arg = " shot" if mode == "producer" else ""
                ...
                try:
                    ...
                    while time.time() - started < 120:
                        ...
                        if any(text in log for text in ("Fatal error:", "Assertion failed:",
                                "CINDERLINE_COMMAND_PREVIEW failed=", "CINDERLINE_COMMAND_PREVIEW refused=")):
    ...
    (OUTPUT / "preview-verification.json").write_text(json.dumps({
        "schema": "cinderline.resource_rally_preview.v1", "cases": results,
        "complete": len(results) == 2, "evidence": "scripted Mac rendering; no native gesture or iPhone acceptance",
