...
model=load_model(model_id);mx.random.seed(42);start=time.monotonic();parts=[]
...
meta=dict(model=model_id,reference=str(ref),reference_origin='User-approved original synthetic Qwen VoiceDesign',reference_text=reftext,script=lines,seed=42,temperature=.9,speed=1,duration=len(a)/r.sample_rate,sample_rate=r.sample_rate,generation_seconds=time.monotonic()-start)
