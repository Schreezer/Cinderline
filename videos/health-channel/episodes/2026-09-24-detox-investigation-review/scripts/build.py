...
raw=json.loads((A/'transcript.json').read_text());raw=raw if isinstance(raw,list) else raw['words']
script=' '.join(json.loads((A/'script-lines.json').read_text())).split()
...
phr=[(0,4),(4,8),(8,13),(13,17),(17,21),(21,24),(24,29),(29,33),(33,37),(37,40),(40,45),(45,49),(49,52),(52,57),(57,60),(60,65),(65,69),(69,72)]
groups=[]
...
cuts=[0,W(8)-.08,W(21)-.08,W(33)-.08,W(40)-.08,W(60)-.08,W(69)-.08,TOTAL]
cuts=[round(c,3) for c in cuts]
assets=['01-office','02-questions','03-answers','04-straightener','05-tea','07-organs','06-ending']
timing={'times':cuts,'audio_start':OFF,'audio_duration':DUR,'total':TOTAL,'gain_db':4.0,'take':'review-s42','script_words':len(script)}
...
for i,(a,b,asset) in enumerate(zip(cuts,cuts[1:],assets),1):
    ...
    if i==5:
        q=loc(W(45));e=loc(W(52))
        special=f'''<div id="{cid}-question" style="position:absolute;left:90px;right:90px;top:1160px;background:#fff8e8;border:5px solid #243a21;border-radius:14px;padding:28px;text-align:center;font:700 53px/1.2 'Source Serif 4',serif;opacity:0">What does it remove?</div><div id="{cid}-evidence" style="position:absolute;left:90px;right:90px;top:1120px;background:#243a21;color:#fff8e8;border-radius:14px;padding:28px;text-align:center;opacity:0"><div style="font:600 27px/1.3 'JetBrains Mono',monospace;color:#e89cb1">DETOX DIETS</div><div style="font:700 49px/1.2 'Source Serif 4',serif;margin-top:14px">Convincing evidence for toxin removal?</div><div id="{cid}-answer" style="font:700 43px/1.2 'JetBrains Mono',monospace;color:#e89cb1;margin-top:14px;opacity:0">LACKING</div></div>'''
        js+=f"tl.fromTo('#{cid}-question',{{y:20,opacity:0}},{{y:0,opacity:1,duration:.3}},{{q}});".replace('{q}',str(q))
        js+=f"tl.to('#{cid}-question',{{opacity:0,duration:.15}},{e}).fromTo('#{cid}-evidence',{{y:20,opacity:0}},{{y:0,opacity:1,duration:.3}},{e+.15}).to('#{cid}-answer',{{opacity:1,duration:.25}},{loc(W(54))});"
    if i==6:
        tl_=round(loc(W(61))-.1,3);tk=round(loc(W(66))-.1,3)
    ...
    if i==7:
        special=f'''<div id="{cid}-end" style="position:absolute;left:120px;right:120px;top:1420px;text-align:center;background:#243a21;color:#efe7d4;padding:22px 20px;font:700 50px/1.15 'Source Serif 4',serif;border-radius:10px;opacity:0" data-layout-ignore="true">Ask for the evidence.</div>'''
...
brand=f'<div id="brand-layer" class="clip" data-start="0" data-duration="{TOTAL}" data-track-index="5" style="z-index:40;pointer-events:none"><div class="brand">FITSNAP · DETOX FILES</div><div class="source">Sources: Sense about Science · NCCIH · NIDDK · NHS</div><div style="position:absolute;left:62px;top:1802px;font:21px/1.4 monospace">Fictional illustrations · AI imagery and narration</div><div class="progress"><div class="progress-inner" id="progress"></div></div></div>'
